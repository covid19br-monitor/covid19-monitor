...
const statesFeatures = topojson.feature(brazilStates, brazilStates.objects.estados);
const statesContour = topojson.mesh(brazilStates, brazilStates.objects.estados);

const maxConfirmed = Math.max.apply(Math, brazilData.map(function(o) { return o.confirmed; }))
...
class Map extends React.Component {
  ...
  _fill = (feature) => {
    const state = brazilData.find((state) => state.name === feature.properties.nome);
  ...
  componentDidMount() {
    ...
    this.svg = d3.select(this.mapWrapper.current).append('svg')
  ...
  render() {
    return <div ref={this.mapWrapper} style={mapWrapperStyle}><div ref={this.tooltip} style={tooltipStyle} /></div>;
  }
}
